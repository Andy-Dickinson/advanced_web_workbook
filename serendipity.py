...
# simple root route, returns 200 status code alongside the response
@app.route("/")
def index():
    return """
...
# responds to both HTTP GET and HTTP POST requests
# when GET arrives, web page is displayed
# displayed page contains simple form with a label, text entry (for question), and button to initiate the POST
# because form submissions are POSTs by default, other part of query() function is executed
# when done, entire page is returned as HTTP response to client
...
def query():
    if request.method == 'POST':
        ...
        # appends new information to page
...
# responds to both HTTP GET and HTTP POST requests. So requires if clause to determine whether
# incoming request is GET / POST
# if GET > creates dictionary, key = 'answer', value = randomly selected 8ball response
# returned by a call to magic8ball() function.
# dictionary then turned into HTTP response containing JSON string as payload
# if POST > extracts question from incoming POST request, adds to dictionary, gets 8ball answer
# and adds to dictionary. Turns that into JSON and returns response to client
...
# responds to HTTP GET requests only.
# creates a dictionary, key = strategy, value = randomly selected oblique strategy returned
# by call to obllique_strategies() function. Dictionary turned into HTTP response containing
...
# Called by magic8ball_page() function
def magic8ball():
    responses = [
        "It is certain.",
        "It is decidedly so.",
